...
Instagram.updateManager = {
  updateUsername: function(a) {
    ...
    document.querySelector(".instagram-url").href = "https://instagram.com/"+user
    document.querySelector("#shareurl").value = window.location.href
    document.querySelector("#embed-website").value = '<iframe height="80px" width="300px" frameborder="0" src="https://livecounts.io/instagram-realtime/embed/?u='+user+'" style="border: 0; width:300px; height:80px; background-color: #FFF;"></iframe>'
    document.querySelector("#obs-url").value = 'https://livecounts.io/instagram-realtime/embed/?u='+user
  },
  ...
  updateFollowingCount: function(a) {
    document.querySelector(".following-odometer").innerHTML= a;
  },
  updateYear: function() {
...
Instagram.refreshManager = {
  start: function() {
    startRefresh = setInterval(function() {
        $.getJSON('https://cors.livecounts.io/https://blastup.com/instagram/info?username='+user, function(data) {
        Instagram.updateManager.updateFollowerCount(parseInt(data.data.followers.replace(/,/g, '')))
        Instagram.updateManager.updateFollowingCount(parseInt(data.data.following.replace(/,/g, '')))
      })
    }, 3000)
  },
  stop: function() {
...
function getData() {
    $.getJSON(Instagram.corsManager.get() + 'https://www.instagram.com/'+user+'/?__a=1', function(data) {
      Instagram.updateManager.updateUsername(data.graphql.user.full_name)
      Instagram.updateManager.updateAvatar(data.graphql.user.profile_pic_url_hd)
      Instagram.updateManager.updateFollowerCount(data.graphql.user.edge_followed_by.count)
      Instagram.updateManager.updateFollowingCount(data.graphql.user.edge_follow.count)
  }).fail(function() {
      setTimeout(function() {
        getData();
      }, 500)
...
function searchUser() {
  ...
  if (change_user.length == 0) {
    ...
    window.location.href = "/instagram-realtime/?u="+change_user
  }
}
...
function searchCompareUser() {
  ...
  if (compare_user.length == 0) {
    ...
    window.location.href = "/instagram-realtime/compare/?user1="+user+"&user2="+compare_user
  }
}
...
setTimeout(function() {
  ...
  getData()
  Instagram.updateManager.updateYear()
  Instagram.refreshManager.start()
},1)


var disqus_config = function() {
    this.page.url = 'https://livecounts.io/instagram-realtime/?u=' + user;
};
