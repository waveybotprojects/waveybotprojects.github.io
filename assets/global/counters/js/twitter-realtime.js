...
window.onload = () => {
	...
	document.getElementById('embed').href = "/twitter-realtime/embed/?u="+user

	$(".links").load("/assets/global/other.html");

	$.getJSON('https://api.thesocialcounter.com/twitter/?name='+user, function(data) {
		Twitter.UpdateManager.updateName(data.name)
		Twitter.UpdateManager.updateAvatar(data.profile_image_url_https.replace("default", "400x400"))
	})
};
...
setInterval(function () {
	$.getJSON('https://bastet.socialblade.com/twitter/lookup?query='+user, function(data) {
		Twitter.UpdateManager.updateOdometer(data)
		Twitter.GoalManager.load(data)
...
Twitter.GoalManager = {
    load: function(a) {
		...
		Twitter.UpdateManager.updateGoal(goal);
    }
}

function search() {
    window.location.href = '/twitter-realtime/?u=' + document.getElementById('search').value.replace(" ", "");
}
